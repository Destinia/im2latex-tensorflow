...
val_results = tf.argmax(tf.nn.softmax(logits),axis=1)
...
def predict(set='test',batch_size=1,visualize=True):
    ...
    idx_to_chars = lambda Y: ' '.join([properties['idx_to_char'][x] for x in Y])
...
def score(set='valid',batch_size=32):
    ...
    for score_imgs,score_seqs,score_mask in score_itr:
        _loss, _output = sess.run([loss, val_results],feed_dict={X:score_imgs,seqs:score_seqs,mask:score_mask})
        losses.append(_loss)
        accuracy.append(wordErrorRate(_output, score_seqs))  # remove BOS

    set_loss = np.mean(losses)
    mean_accuracy = sum(accuracy)/len(accuracy)
    perp = np.mean([np.power(np.e,x) for x in losses])
    print("\tMean %s Loss: ", set_loss)
    print("\tTotal %s Time: ", time.time()-start)
    print("\tMean %s Perplexity: ", perp)
    print("\tMean %s Accuracy: ", mean_accuracy)
...
for i in range(i,NB_EPOCHS):
    ...
    print("\tMean train perplexity: ",np.mean([np.power(np.e,x) for x in costs]))
    print("\tMean time: ",np.mean(times))
    val_loss, val_perp = score('valid',BATCH_SIZE)
    if val_perp < best_perp:
        best_perp = val_perp
        saver.save(sess, "./save/weights_best.ckpt")
